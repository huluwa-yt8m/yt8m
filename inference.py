...
if __name__ == "__main__":
  ...
  flags.DEFINE_string("segment_label_ids_file", "gs://lijupan12_yt8m_train_bucket/yt8m_train_frame_level_logistic_model/segment_label_ids.csv",
  ...
  flags.DEFINE_string(
      ...
      "top 10 participants.")
  flags.DEFINE_integer("top_k", 80, "How many predictions to output per video.")
...
def format_lines(video_ids, predictions, top_k, whitelisted_cls_mask=None):
  ...
  if whitelisted_cls_mask is not None:
    logging.info("whitelisted_cls_mask is OK")
    ...
    if FLAGS.segment_labels:
      ...
      if FLAGS.segment_label_ids_file:
        whitelisted_cls_mask = np.zeros((predictions_tensor.get_shape()[-1],),
                                        dtype=np.float32)
        with file_io.FileIO(FLAGS.segment_label_ids_file, mode='rb') as fobj:
          for line in fobj:
            try:
              cls_id = int(line)
              ...
              logging.info("Simply skip the non-integer line: "+line)
              continue
